...
for py in cp27-cp27mu cp35-cp35m cp36-cp36m cp37-cp37m; do
  ...
  export PYTHON=/opt/python/$py/bin/python;
  cd /tmp;
  $PYTHON -m pip uninstall -y prob_phoc;
  $PYTHON -m pip install prob_phoc --no-index -f /tmp/src/dist --no-dependencies -v;
  $PYTHON -m unittest prob_phoc.test;
  cd - 2>&1 > /dev/null;
done;

set +x;
ODIR="/host/tmp/prob_phoc/whl/${CUDA_VERSION_S}";
mkdir -p "$ODIR";
